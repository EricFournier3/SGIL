...
ERROR_FILE="FAX_ERROR_${CURRENT_YEAR}${MOIS}${JOUR}_${CURRENT_HOUR}.txt"
ERROR_FILE_PATH="${CURRENT_PATH}${ERROR_FILE}"
...
if [ ! -f ${FAXLOG_PATH} ]
  then
  echo "Impossible de trouver le fichier ${FAXLOG_PATH}"
  exit 1
fi
...
awk '/BUSY/{if($5 > 4){print $0}}' ${FAXLOG_PATH} | sed -n "/${JOUR}\/${MOIS}/p" > ${ERROR_FILE_PATH}  
...
sudo cp ${ERROR_FILE_PATH} $BASEDIR_OUT


rm ${FAXSTATUS_PATH_CURRENT} ${STAT_FILE_PATH} ${ERROR_FILE_PATH}
